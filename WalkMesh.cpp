...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
	...
	auto adj_tri_check = next_vertex.find(edge); 
	if (adj_tri_check != next_vertex.end()) {
		uint32_t next_vertex = adj_tri_check->second;

		//make 'end' represent the same (world) point, but on triangle (edge.y, edge.x, [other point]):
		//TODO
		end.indices = glm::vec3(edge.y, edge.x, next_vertex);
		...
		glm::vec3 current_tri_normal = glm::normalize(glm::cross(b - a, c - a));

		glm::vec3 adj_a = vertices[edge.y];
		glm::vec3 adj_b = vertices[edge.x];
		glm::vec3 adj_c = vertices[next_vertex];
		glm::vec3 adj_tri_normal = glm::normalize(glm::cross(adj_b - adj_a, adj_c - adj_a));

		rotation = glm::rotation(current_tri_normal, adj_tri_normal);
		return true;
	} else {
		end = start;
